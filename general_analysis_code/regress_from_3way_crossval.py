from general_analysis_code.regress_from_2way_crossval import ridge_custom_groups
...
def regress_from_3way_crossval_himalaya(X, Y, groups,alphas= np.logspace(-100, 100, 201,base=2), save_parameters=False,half=False):
    ...
    for index,test_fold in enumerate(all_folds):
        ...
        model,best_alphas = ridge_custom_groups(F_train, Y_train, train_fold_indices,alphas=alphas,refit=True,autocast=half)
...
def regress_from_3way_crossval_sam(X, y, groups,alphas= np.logspace(-100, 100, 201,base=2), save_parameters=False):
    ...
    y_hat, _, _, _, _, B = regress_predictions_from_3way_crossval(
            X,
            y,
            test_folds=groups,
            train_folds=groups,
...
def regress_from_3way_crossval(X, Y, groups,alphas= np.logspace(-100, 100, 201,base=2), save_parameters=True,backend='himalaya',half=False):
    if backend == 'himalaya':
        return regress_from_3way_crossval_himalaya(X, Y, groups,alphas=alphas, save_parameters=save_parameters,half=half)
    elif backend == 'sam':
